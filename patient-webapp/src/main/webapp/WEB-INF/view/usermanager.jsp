			...
			<div class="col-xs-6 text-left">
				<ul class="pagination panel-pagination usermanager-pagination">
					<li<c:if test="${prev_offset < 0}"> class="disabled"</c:if>>
						<a href="/usermanager?o=${prev_offset}">
							&#60;&#60; <spring:message code="label.previous" />
						</a>
					</li>
					<li<c:if test="${end}"> class="disabled"</c:if>>
						<a href="/usermanager?o=${next_offset}">
		...
		<table class="table table-striped table-hover">
			...
			<c:forEach items="${userList}" var="user">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${user.active}"><spring:message code="label.yes" /></c:when>
							<c:otherwise><spring:message code="label.no" /></c:otherwise>
						</c:choose>
					</td>
					<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${user.createDate}" /></td>
					<td>
						<button type="button" class="btn btn-xs btn-primary" onclick="location.href='/usermanager/addUser?row=${user.id}'">
